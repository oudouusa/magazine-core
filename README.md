...
- **`1.3.0` released**（2026-08-31、stable `1.x` 系）。trusted pluginのcommand
  manifestに、値を保存せず明示名だけをparent環境から継承する`env_from`を追加した。tag と
...
- stable tag / Release の作成は `docs/release/1.3.0.md` と
...
repo paths. `1.3.0` is the current stable tag.

```bash
RELEASE_TAG=1.3.0 VERIFY_UI=1 VERIFY_TRUSTED_UI=1 bash scripts/verify-standalone-quickstart.sh
```
