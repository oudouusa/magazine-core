...
- current implementation line: protocol、domain / DB / CLI、plugin host runtime、
  ...
  extension parity evidence では、protocol v1 contract / code の追加変更は不要と
  監査済み。conformance fixtures は beta readiness 向けに拡張済み。Python SDK root
  plugin-author API も `0.1.0-beta` 向けに凍結済み。
- conformance fixture inventory は、typed state golden と non-empty `page_urls` golden を含めて完了。
