...
fn run_discover(args: &[String]) -> Result<(), Box<dyn Error>> {
    ...
    let options = parse_discover_options(&args[3..])?;
...
fn parse_discover_options(args: &[String]) -> Result<DiscoverOptions, Box<dyn Error>> {
...
fn parse_timeout_seconds(value: &str) -> Result<Duration, Box<dyn Error>> {
    let parsed = value
        .parse::<u64>()
        .map_err(|_| -> Box<dyn Error> { "--timeout-seconds must be a positive integer".into() })?;
    if parsed == 0 {
...
"#,
    ...
    #[test]
    fn discover_timeout_option_controls_deadline_and_remaining_ms() {
        let dir = temp_dir("discover-timeout");
        let db_path = dir.join("core.db");
        let remaining_file = dir.join("remaining-ms.txt");
...
discover = read_frame()
with open(os.environ["REMAINING_FILE"], "w") as f:
    f.write(str(discover["params"]["remaining_ms"]))
time.sleep(float(os.environ["SLEEP_SECONDS"]))
write_frame({"jsonrpc": "2.0", "id": discover["id"], "result": {"records": 0}})
"#,
        )
        .unwrap();
        ...
        let remaining_ms: u64 = fs::read_to_string(&remaining_file)
            .unwrap()
            .parse()
            .unwrap();
        assert!(
            remaining_ms <= 5_000,
            "remaining_ms {remaining_ms} did not reflect the 5s timeout"
        );
        ...
        .unwrap_err();
        assert!(
            err.to_string().contains("plugin timed out"),
    ...
    #[test]
    fn parse_discover_options_accepts_optional_flags() {
        let options = parse_discover_options(&[
            "--max-pages".to_string(),
            "2".to_string(),
            "--max-records".to_string(),
            "3".to_string(),
    ...
    #[test]
    fn parse_discover_options_rejects_unknown_duplicate_and_invalid_values() {
        assert!(parse_discover_options(&["--unknown".to_string(), "1".to_string()]).is_err());
        assert!(
            parse_discover_options(&["--max-pages".to_string(), "--per-page".to_string()]).is_err()
        );
        ...
        .is_err());
        assert!(parse_discover_options(&["--per-page".to_string(), "-1".to_string()]).is_err());
    ...
    #[test]
    fn parse_discover_options_accepts_timeout_and_rejects_invalid_values() {
        let options = parse_discover_options(&[
            "--max-pages".to_string(),
            "2".to_string(),
            "--timeout-seconds".to_string(),
        ...
        assert_eq!(options.limits.max_pages, Some(2));
        assert_eq!(options.timeout, std::time::Duration::from_secs(5));
        assert!(
            parse_discover_options(&["--timeout-seconds".to_string(), "0".to_string()]).is_err()
        );
        assert!(
            parse_discover_options(&["--timeout-seconds".to_string(), "-1".to_string()]).is_err()
        );
        assert!(parse_discover_options(&[
            "--timeout-seconds".to_string(),
            "1".to_string(),
